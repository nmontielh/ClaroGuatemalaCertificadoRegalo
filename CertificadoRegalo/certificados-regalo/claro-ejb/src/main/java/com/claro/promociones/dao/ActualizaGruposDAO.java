...
public class ActualizaGruposDAO
implements Runnable {
    ...
    public ActualizaGruposDAO(FileDataTO fileDataTO, int idRegion) {
        try {
            ...
            this.error.error((Object)"ActualizaGruposDAO", (Throwable)e);
        }
    }

    public boolean actualizaGrupos() throws Exception {
        ...
        File update = null;
        File error = null;
        FileOutputStream updateOutput = null;
        ...
        try {
            ...
            try {
                ...
                String logUpdates = "gpoActualizadosR" + this.idRegion + ".txt";
                String logErrores = "gpoActualizadosErroresR" + this.idRegion + ".txt";
                update = new File(logUpdates);
                error = new File(logErrores);
                updateOutput = new FileOutputStream(update);
                errorOutput = new FileOutputStream(error);
                reporteInsert = new PrintWriter(updateOutput);
                ...
                while (linea != null) {
                    ...
                    try {
                        GrupoTO grupoActualizado = this.actualizar(grupoTO, connection);
                        if (grupoActualizado.getIdError() == 0) {
                            connection.commit();
                            reporteInsert.println(String.valueOf(++movimientosBD) + ": Se actualizo[" + grupoActualizado.toString() + "]");
                            continue;
                        }
                        reporteError.println(String.valueOf(++movimientosBD) + ": No se actualizo[" + grupoActualizado.toString() + "]idError:" + grupoActualizado.getIdError() + ",Mensaje:" + grupoActualizado.getMensaje());
                        continue;
                    }
                    catch (Exception e) {
                        reporteError.println(String.valueOf(++movimientosBD) + ": No se actualizo[" + grupoTO.toString() + "]idError:" + grupoTO.getIdError() + ",Mensaje:" + grupoTO.getMensaje());
                    }
                }
                if (movimientosBD > 0) {
                    connection.commit();
                ...
                archivosLog.add(logUpdates);
                archivosLog.add(logErrores);
                Utils.enviaCorreo(parametrosCorreo, (String)"Actualizacion de grupos de promociones.", (String)"Los grupos de promociones fueron procesados.", archivosLog);
            }
     ...
     */
    private GrupoTO actualizar(GrupoTO grupoTO, Connection connection) {
