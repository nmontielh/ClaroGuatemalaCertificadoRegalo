...
public class CertificadosDAO {
			...
			try {
				try {
					...
					query.append("FROM  ").append(this.schema_database)
							.append("CC_TBLMOVIMIENTOCERTIFICADO ");
					...
					while (resultSet.next()) {
						...
						movimientoCertificadoTO.setNumeroCertificado(resultSet
								.getString("NUMCERTIFICADO").trim());
						movimientoCertificadoTO.setNumeroTarjeta(resultSet
								.getString("NUMTARJETA").trim());
						movimientoCertificadoTO.setIdMotivo(resultSet
								.getString("IDMOVTO").trim());
						movimientoCertificadoTO.setIdUsuario(resultSet
								.getString("IDUSUARIO").trim());
						movimientoCertificadoTO.setPuntoVenta(resultSet
								.getString("PUNTOVTA").trim());
						movimientoCertificadoTO.setFechaOperacion(String
								.valueOf(resultSet.getTimestamp("FECHAOPER")));
						movimientoCertificadoTO.setEstatus(resultSet.getString(
								"ESTATUS").trim());
						movimientoCertificadoTO.setValorAplicado(resultSet
								.getFloat("VALORAPLICADO"));
						movimientoCertificadoTO.setValorAnterior(resultSet
								.getFloat("VALORANTERIOR"));
						movimientoCertificadoTO.setValorRestante(resultSet
								.getFloat("VALORRESTANTE"));
						movimientoCertificadoTO.setReferencia(resultSet
								.getString("REFERENCIA").trim());
	...
	/**
	 * @see refactor CONSULTA MOVIMIENTO
		...
		block29: {
			...
			try {
				try {
					...
					query.append("FROM  ").append(this.schema_database)
							.append("CC_TBLTARJETACERTIFICADO ");
					...
					resultSet = stmt.executeQuery();
					if (resultSet.next()) {
						tarjetaCertificadoTO.setNumeroTarjeta(resultSet
								.getString("NUMTARJETA").trim());
						tarjetaCertificadoTO.setNumeroCertificado(resultSet
								.getString("NUMCERTIFICADO").trim());
						tarjetaCertificadoTO.setSaldo(resultSet
								.getFloat("VALORRESTANTE"));
						tarjetaCertificadoTO.setFechaActivacion(String
								.valueOf(resultSet
										.getTimestamp("FECHAACTIVACION")));
						tarjetaCertificadoTO.setFechaExpiracion(String
								.valueOf(resultSet
										.getTimestamp("FECHAEXPIRACION")));
						tarjetaCertificadoTO.setEstatus(resultSet.getString(
								"ESTATUS").trim());
					...
					tarjetaCertificadoTO
							.setMensaje("Certificado no v\u00e1lido.");
				} catch (Exception e) {
	...
	public ActivacionTarjetaCertificadoTO activaTarjetaCertificado(
		...
		block49: {
			...
			block50: {
				...
				activacionTarjetaCertificadoTO
						.setMensaje("No se pueden activar certificados ya que usuario generico no existe");
			...
			try {
				...
				if (Certificado != null) {
					if (Estatus.trim().equals("N")) {
						...
						query.append(this.schema_database)
								.append("CC_TBLTARJETACERTIFICADO(NUMCERTIFICADO,NUMTARJETA, ");
						...
						stmt.setTimestamp(
								4,
								new Timestamp(fechaActivacion.getTimeInMillis()));
						stmt.setTimestamp(
								5,
								new Timestamp(fechaExpiracion.getTimeInMillis()));
						stmt.setString(6, "1");
						...
						activacionTarjetaCertificadoTO
								.setNumeroTarjeta(numeroTarjeta.trim());
						activacionTarjetaCertificadoTO
								.setNumeroCertificado(Certificado.trim());
						activacionTarjetaCertificadoTO
								.setMontoCertificado(montoCertificado);
						SimpleDateFormat sdf = new SimpleDateFormat("ddMMyyyy");
						String fechaAct = sdf.format(fechaActivacion.getTime());
						String fechaExp = sdf.format(fechaExpiracion.getTime());
						activacionTarjetaCertificadoTO
								.setFechaActivacion(fechaAct);
						activacionTarjetaCertificadoTO
								.setFechaExpiracion(fechaExp);
						activacionTarjetaCertificadoTO.setEstatus("1");
						query = new StringBuilder();
						query.append("UPDATE ").append(this.schema_database)
								.append("CC_TBLCERTIFICADO ");
						...
						activacionTarjetaCertificadoTO
								.setMensaje("Se activo el certificado correctamente");
						return activacionTarjetaCertificadoTO;
					}
					activacionTarjetaCertificadoTO.setIdMensaje("03");
					activacionTarjetaCertificadoTO.setMensaje("La tarjeta: "
							+ numeroTarjeta + " ya fue vendida");
					return activacionTarjetaCertificadoTO;
				}
				activacionTarjetaCertificadoTO.setIdMensaje("03");
				activacionTarjetaCertificadoTO
						.setMensaje("No existe la tarjeta: " + numeroTarjeta
								+ " a Activar o el monto: " + montoCertificado
								+ " es invaldo.");
				return activacionTarjetaCertificadoTO;
			} catch (Exception e) {
				if (conn != null) {
				...
				activacionTarjetaCertificadoTO.setMensaje("Error: "
						+ e.toString());
				if (resultSet != null) {
					try {
						resultSet.close();
	 ...
	 */
	/**
	 * @see refactor APLICA CERTIFICADO
	...
	public MensajeServiceTO aplicaCertificado(
		...
		try {
			try {
				...
				query.append("WHERE NUMCERTIFICADO = ? ");
				stmt = conn.prepareStatement(query.toString());
				stmt.setString(1, movimientoCertificadoTO
						.getNumeroCertificado().trim());
				resultSet = stmt.executeQuery();
				...
				if (resultSet.next()) {
					...
					tarjetaCertificadoTO.setNumeroCertificado(resultSet
							.getString("NUMCERTIFICADO").trim());
					tarjetaCertificadoTO.setNumeroTarjeta(resultSet.getString(
							"NUMTARJETA").trim());
					tarjetaCertificadoTO.setSaldo(resultSet
							.getFloat("VALORRESTANTE"));
					tarjetaCertificadoTO.setFechaActivacion(sdf
							.format(resultSet.getTimestamp("FECHAACTIVACION")));
					tarjetaCertificadoTO.setFechaExpiracion(sdf
							.format(resultSet.getTimestamp("FECHAEXPIRACION")));
					tarjetaCertificadoTO.setEstatus(resultSet.getString(
							"ESTATUS").trim());
				}
				if (resultSet != null) {
				...
				if (tarjetaCertificadoTO != null) {
					if (tarjetaCertificadoTO.getEstatus().equals("1".trim())) {
						Date fechaExpiracion = sdf.parse(tarjetaCertificadoTO
								.getFechaExpiracion());
						Date hoy = new Date();
						if ((hoy = sdf.parse(sdf.format(hoy)))
								.compareTo(fechaExpiracion) <= 0) {
							if (tarjetaCertificadoTO.getSaldo() >= movimientoCertificadoTO
									.getValorAplicado()) {
								query = new StringBuilder();
								query.append("INSERT INTO ");
								query.append(this.schema_database)
										.append("CC_TBLMOVIMIENTOCERTIFICADO (FOLIO,NUMCERTIFICADO, ");
								...
								mensajeServiceTO
										.setFolio(this
												.getFolioMovimientoCertificado(tarjetaCertificadoTO
														.getNumeroCertificado()
														.trim()));
								stmt = conn.prepareStatement(query.toString());
								stmt.setString(1, mensajeServiceTO.getFolio()
										.trim());
								stmt.setString(2, tarjetaCertificadoTO
										.getNumeroCertificado().trim());
								stmt.setString(3, tarjetaCertificadoTO
										.getNumeroTarjeta().trim());
								stmt.setString(4, "1");
								stmt.setString(5, movimientoCertificadoTO
										.getIdUsuario().trim());
								stmt.setString(6, movimientoCertificadoTO
										.getPuntoVenta().trim());
								stmt.setTimestamp(
										7,
										new Timestamp(System
												.currentTimeMillis()));
								stmt.setString(8, "A");
								stmt.setFloat(9, movimientoCertificadoTO
										.getValorAplicado());
								stmt.setFloat(10,
										tarjetaCertificadoTO.getSaldo());
								float valorRestante = tarjetaCertificadoTO
										.getSaldo()
										- movimientoCertificadoTO
												.getValorAplicado();
								stmt.setFloat(11, valorRestante);
								stmt.setString(12, movimientoCertificadoTO
										.getReferencia().trim());
								stmt.executeUpdate();
								if (resultSet != null) {
									resultSet.close();
								...
								mensajeServiceTO
										.setMensaje("Se realizo satisfactoriamente la operaci\u00f3n. Saldo Restante: "
												+ valorRestante);
								return mensajeServiceTO;
							} else {
								mensajeServiceTO.setId("05");
								mensajeServiceTO
										.setMensaje("No cuenta con saldo suficiente, saldo: "
												+ tarjetaCertificadoTO
														.getSaldo());
							}
							return mensajeServiceTO;
							...
							mensajeServiceTO
									.setMensaje("El certificado ha expirado, fecha de expiracion: "
											+ sdf.format(fechaExpiracion));
						}
						return mensajeServiceTO;
					} else {
						if (tarjetaCertificadoTO.getEstatus().equals("2")) {
							...
							mensajeServiceTO
									.setMensaje("Error, ha caducado la vigencia del certificado.");
						}
						if (!tarjetaCertificadoTO.getEstatus().equals("3"))
							return mensajeServiceTO;
						mensajeServiceTO.setId("3");
						mensajeServiceTO
								.setMensaje("Error, el certificado esta cancelado.");
					}
					return mensajeServiceTO;
				}
				mensajeServiceTO.setId("01");
				mensajeServiceTO
						.setMensaje("No existe activacion para el certificado: "
								+ movimientoCertificadoTO
										.getNumeroCertificado().trim());
				return mensajeServiceTO;
			} catch (Exception e) {
	...
	private String getFolioMovimientoCertificado(String numeroCertificado) {
		...
		Date date = new Date();
		String folio = String.valueOf(numeroCertificado.substring(
				numeroCertificado.length() - 4, numeroCertificado.length()))
				+ sdf.format(date);
		return folio.trim();
	}

	/**
	 * @ee refactor CANCELA ACTIVACION TARJETA
	 * @param numeroCertificado
	 * @param idUsuario
	 * @return
	 * @throws ClaroException
			...
			try {
				try {
					...
					if (resultSet.next()) {
						...
						cancelaCertificadoTO.setNumeroCertificado(resultSet
								.getString("NUMCERTIFICADO").trim());
						cancelaCertificadoTO.setNumeroTarjeta(resultSet
								.getString("NUMTARJETA").trim());
						cancelaCertificadoTO.setSaldo(resultSet
								.getFloat("VALORRESTANTE"));
						cancelaCertificadoTO.setEstatusTarjeta(resultSet
								.getString("ESTATUS").trim());
					} else {
					...
					if (cancelaCertificadoTO != null) {
						...
						stmt = conn.prepareStatement(query.toString());
						stmt.setString(1,
								cancelaCertificadoTO.getNumeroCertificado());
						resultSet = stmt.executeQuery();
						if (resultSet.next()) {
							cancelaCertificadoTO.setSaldoinicial(resultSet
									.getFloat("VALOR"));
							cancelaCertificadoTO
									.setEstatusCertificado(resultSet.getString(
											"APLICADO").trim());
						} else {
							cancelaCertificadoTO = null;
						}
					...
					if (cancelaCertificadoTO != null) {
						if (cancelaCertificadoTO.getSaldo() >= cancelaCertificadoTO
								.getSaldoinicial()) {
							if (!cancelaCertificadoTO.getEstatusTarjeta()
									.trim().equals("2".trim())) {
								if (cancelaCertificadoTO
										.getEstatusCertificado().trim()
										.equals("S".trim())) {
									query = new StringBuilder();
									query.append("UPDATE ")
											.append(this.schema_database)
											.append("CC_TBLTARJETACERTIFICADO SET ESTATUS= ?, IDUSUARIO = ?");
									query.append("WHERE NUMTARJETA = ? AND NUMCERTIFICADO = ? ");
									stmt = conn.prepareStatement(query
											.toString());
									stmt.setString(1, "3");
									stmt.setString(2, idUsuario);
									stmt.setString(3, cancelaCertificadoTO
											.getNumeroTarjeta().trim());
									stmt.setString(4, cancelaCertificadoTO
											.getNumeroCertificado().trim());
									stmt.executeUpdate();
									if (resultSet != null) {
										resultSet.close();
									...
									stmt.setString(2, cancelaCertificadoTO
											.getNumeroCertificado().trim());
									stmt.executeUpdate();
									mensajeServiceTO.setId("00".trim());
									mensajeServiceTO
											.setMensaje("Se cancelo el certificado: "
													+ cancelaCertificadoTO
															.getNumeroTarjeta()
															.trim());
									conn.commit();
									...
									mensajeServiceTO.setId("01");
									mensajeServiceTO
											.setMensaje("No se puede cancelar el certificado por que no ha sido vendido");
								}
							} else {
								mensajeServiceTO.setId("06");
								mensajeServiceTO
										.setMensaje("No se puede cancelar el certificado ya que ha caducado");
							}
						} else {
							mensajeServiceTO.setId("01");
							mensajeServiceTO
									.setMensaje("El certificado "
											+ cancelaCertificadoTO
													.getNumeroTarjeta().trim()
											+ " ha sido utilizado y no puede ser cancelado");
						}
						break block49;
					}
	...
	/**
	 * @see refactor CANCELA CERTIFICADO
	 * @param folio
	 * @param idUsuario
	 * @param idpuntoVta
	 * @param referencia
			...
			try {
				try {
					...
					if (resultSet.next()) {
						...
						cancelaMovimientoTO.setNumeroCertificado(resultSet
								.getString("NUMCERTIFICADO"));
						cancelaMovimientoTO.setNumeroTarjeta(resultSet
								.getString("NUMTARJETA"));
						cancelaMovimientoTO.setSaldo(resultSet
								.getFloat("VALORAPLICADO"));
						cancelaMovimientoTO.setFechaOperacion(sdf
								.format(resultSet.getTimestamp("FECHAOPER")));
						cancelaMovimientoTO.setEstatusMovto(resultSet
								.getString("ESTATUS"));
					}
					if (resultSet != null) {
						resultSet.close();
					...
					if (cancelaMovimientoTO != null) {
						...
						stmt = conn.prepareStatement(query.toString());
						stmt.setString(1,
								cancelaMovimientoTO.getNumeroTarjeta());
						resultSet = stmt.executeQuery();
						TarjetaCertificadoTO tarjetaCertificadoTO = null;
						if (resultSet.next()) {
							tarjetaCertificadoTO = new TarjetaCertificadoTO();
							...
							tarjetaCertificadoTO.setFechaExpiracion(sdf
									.format(resultSet
											.getTimestamp("FECHAEXPIRACION")));
							tarjetaCertificadoTO.setEstatus(resultSet
									.getString("ESTATUS"));
						}
						if (resultSet != null) {
							resultSet.close();
						...
						if (tarjetaCertificadoTO != null) {
							if (Utils
									.diferenciaDiasCertificado((String) cancelaMovimientoTO
											.getFechaOperacion()) <= 0) {
								if (tarjetaCertificadoTO.getEstatus().equals(
										"1")) {
									if (!cancelaMovimientoTO.getEstatusMovto()
											.equals("I")) {
										query = new StringBuilder();
										query.append("UPDATE ")
												.append(this.schema_database)
										...
										stmt.setTimestamp(7, new Timestamp(
												System.currentTimeMillis()));
										stmt.setString(8, "A");
										stmt.setFloat(9,
												cancelaMovimientoTO.getSaldo());
										stmt.setFloat(10,
												tarjetaCertificadoTO.getSaldo());
										stmt.setFloat(11, valorRestante);
										...
										mensajeServiceTO.setId("00");
										mensajeServiceTO
												.setMensaje("El movimiento fue cancelado exitosamente.");
									} else {
										mensajeServiceTO.setId("01");
										mensajeServiceTO
												.setMensaje("El movimiento ya fue cancelado.");
									}
								} else {
									mensajeServiceTO.setId("01");
									mensajeServiceTO
											.setMensaje("El certificado tiene un estatus diferente a activo");
								}
							} else {
								mensajeServiceTO.setId("01");
								mensajeServiceTO
										.setMensaje("El movimiento tiene mas de 1 dia, no puede ser cancelado");
							}
						} else {
							mensajeServiceTO.setId("02");
							mensajeServiceTO
									.setMensaje("No existe informacion del certificado de regalo");
						}
						break block50;
					}
					mensajeServiceTO.setId("01");
					mensajeServiceTO
							.setMensaje("No existen movimientos relacionados al folio");
				} catch (Exception e) {
					if (conn != null) {
						try {
							conn.rollback();
		...
		block29: {
			...
			try {
				try {
					...
					query.append(this.schema_database).append(
							"CC_TBLCERTIFICADO ");
	...
	public List<MovimientoCertificadoTO> consultaMovimientosCertificado(
			String numeroTarjeta) throws ClaroException {
		ArrayList<MovimientoCertificadoTO> movimientos;
		block13: {
			movimientos = new ArrayList<MovimientoCertificadoTO>();
			Connection conn = null;
	...
	public String consultaTajetaVendida(String numeroTarjeta,
		...
		try {
			try {
				...
				stmt.setFloat(2, montoCertificado);
				resultSet = stmt.executeQuery();
				existe = resultSet.next() ? (resultSet.getString(1).equals("N") ? "LA TARJETA NO HA SIDO VENDIDO"
						: (resultSet.getString(1).equals("S") ? "LA TARJETA HA SIDO VENDIDO"
								: "LA TARJETA HA SIDO CANCELADA"))
						: "02|LA TARJETA INGRESADA NO EXISTE";
			} catch (Exception e) {
