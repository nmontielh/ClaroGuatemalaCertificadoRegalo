...
public class Utils extends Constantes {
	protected static final Logger error = Logger
	...
	public static String formatFecha(String valor) {
		...
		if (valor.substring(6, 7).equals("9")) {
			return "19" + valor.substring(6, 8) + "-" + valor.substring(3, 5)
					+ "-" + valor.substring(0, 2);
		}
		return "20" + valor.substring(6, 8) + "-" + valor.substring(3, 5) + "-"
				+ valor.substring(0, 2);
	}

	public static boolean getValEstatusTel(String sEstatus, String sMotivo) {
		boolean bResp = false;
		if (!(sEstatus == null || sMotivo == null
				|| sEstatus.trim().equals("AC") || sEstatus.trim().equals("SU")
	...
	public static int getMesActual(String fecPzoFzo) {
		...
		int inCurYear = Integer.parseInt(Utils.getFechaInsert()
				.substring(6, 10));
		...
		inMesActual = inCurYear == inAdYear ? inCurMon - inAdMon
				: (inCurMon <= inAdMon ? inAnioActual - (inAdMon - inCurMon)
						: inAnioActual + (inCurMon - inAdMon));
		if (inAdDay > inCurDay) {
			--inMesActual;
		}
		return inMesActual;
	...
	public static boolean terminoAdendum(String fecAde, String pzo, int region,
			String sRedencionAntc) {
		...
		anioVen = mesAde == 12 ? Integer.toString((plazo + mesAde - 1) / 12
				+ anioAde) : Integer.toString((plazo + mesAde) / 12 + anioAde);
		...
		resp = region == 9 ? (difDias <= 25 ? true : difDias > 25
				&& difDias <= 90 && "1".equals(sRedencionAntc))
				: (difDias <= 15 ? true : difDias > 15 && difDias <= 180
	...
	public static boolean getValEstatusCobranza(String sEstatusCob, int nReg,
			String sBoton, String sMotivo, String sStatusTel) {
		boolean bResp = false;
		...
		if (sBoton.trim().equals("Saldos")
				&& nReg != 9
				&& (sEstatusCob.trim().equals("EM")
						|| sEstatusCob.trim().equals("HL")
						|| sEstatusCob.trim().equals("SP")
						|| sEstatusCob.trim().equals("AR")
						|| sEstatusCob.trim().equals("ET")
						|| sEstatusCob.trim().equals("RE")
						|| sEstatusCob.trim().equals("SL")
	...
	public static boolean ValidaCredito(String sClase, int region,
		...
		if ((sClase != null || perfilTO != null) && sClase.trim().equals("AR")) {
			tienePrivilegios = SeguridadCaUtil.getInstance()
	...
	public static boolean defineIva(String tipoReden, PerfilTO perfilTO) {
		...
		if (tipoReden.equals("SIN")) {
			resp = SeguridadCaUtil.getInstance().validaPerfilProcesoCa(
					perfilTO, "138");
		}
	...
	public static int comparaFechas(ArrayList<String> vFechas, int indice1,
			int indice2) {
		String lFecAlta1 = "";
		String lFecAlta2 = "";
		try {
	...
	public static String validaResultSet(ResultSet resultSet, String campo,
			Class tipo) throws CAException {
		String valor = "";
		try {
			...
			throw new CAException(-1, "SQLException.validaResultSet ["
					+ e.toString() + "]", e);
		} catch (Exception e) {
	...
	public static int getMesFecha(String fvenc2, String freden) {
		...
		int intAnioTot = 0;
		intAnioTot = inAnioActual == 1 || inAnioActual == 0 ? 12
				: inAnioActual * 12;
		inMesActual = inYearvenc2 == inAdYear ? inAdMon - inMonvenc2
				: (inAdMon <= inMonvenc2 ? intAnioTot - (inMonvenc2 - inAdMon)
						: intAnioTot + (inAdMon - inMonvenc2));
	...
	public static String generaFolio(String telefono) {
		String folio = "";
		folio = String.valueOf(Constantes.yyyyMMddHHmmss.format(
				new java.util.Date()).substring(2, 13))
	...
	public static boolean validaFechaActivacion(String fechaAltaM2K) {
		try {
			...
			fechaAlta.setTime(Constantes.DATEFORMATyyyy_MM_dd
					.parse(fechaAltaM2K));
			if (fechaAlta.get(2) == fechaActual.get(2) - 1
					&& fechaAlta.get(1) == fechaActual.get(1) - 1
					|| fechaAlta.get(2) == fechaActual.get(2)
					&& fechaAlta.get(1) == fechaActual.get(1)) {
				return false;
			}
			return true;
		} catch (Exception e) {
	...
	public static String generaFolioAlianza(String telefono) {
		...
		SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyyMMddhhmmss",
				currentLocale);
		folio = String.valueOf(dateFormatter.format(new java.util.Date())
				.substring(3, 14)) + telefono.substring(2, 10);
		return folio;
	}
	...
	public static PuntosRedimidosTO regresaPuntos(PuntosTO puntosTO,
		...
		redimidosTO.setPtsPorVencer(puntosTO.getPtsPorVencer()
				+ puntosRedimidosTO.getPtsPorVencer());
		redimidosTO.setPtsPorVencer1(puntosTO.getPtsPorVencer1()
				+ puntosRedimidosTO.getPtsPorVencer1());
		redimidosTO.setPtsPorVencer2(puntosTO.getPtsPorVencer2()
		...
		redimidosTO.setPtsRedimidos(puntosTO.getPtsRedimidos()
		...
		redimidosTO.setBonoEquipo(puntosTO.getBonoEquipo()
	...
	public static PuntosRedimidosTO canjePuntos(PuntosTO puntosTO,
		...
		if (rubros <= 7 && puntosTO.getPtsPorVencer() > 0) {
			if (puntosTO.getPtsPorVencer() <= valorPuntos) {
				...
				redimidosTO
						.setPtsPorVencerRedimidos(puntosTO.getPtsPorVencer());
		...
		if (rubros <= 7 && puntosTO.getPtsPorVencer1() > 0) {
			if (puntosTO.getPtsPorVencer1() <= valorPuntos) {
				...
				redimidosTO.setPtsPorVencer1Redimidos(puntosTO
						.getPtsPorVencer1());
		...
		if (rubros <= 7 && puntosTO.getPtsPorVencer2() > 0) {
			if (puntosTO.getPtsPorVencer2() <= valorPuntos) {
				...
				redimidosTO.setPtsPorVencer2Redimidos(puntosTO
						.getPtsPorVencer2());
		...
		if (rubros <= 7 && puntosTO.getPtsPromocion() > 0) {
			if (puntosTO.getPtsPromocion() <= valorPuntos) {
				...
				redimidosTO
						.setPtsPromocionRedimidos(puntosTO.getPtsPromocion());
		...
		if (rubros <= 7 && puntosTO.getPtsAntiguedad() > 0) {
			if (puntosTO.getPtsAntiguedad() <= valorPuntos) {
				...
				redimidosTO.setPtsPorAntiguedadRedimidos(puntosTO
						.getPtsAntiguedad());
		...
		if (rubros <= 7 && puntosTO.getPtsExcedentes() > 0) {
			if (puntosTO.getPtsExcedentes() <= valorPuntos) {
				...
				redimidosTO.setPtsExcedentesRedimidos(puntosTO
						.getPtsExcedentes());
	...
	public static long diferenciaDiasTransf(String _fecAlta) {
		...
		try {
			Locale currentLocale = new Locale("en", "US");
			SimpleDateFormat dateFormatAlta = new SimpleDateFormat(
	...
	public static int calculaPuntos(String lFecAlta) {
		...
		try {
			java.util.Date fechaAlta = new java.util.Date(
					Constantes.DATEFORMATyyyy_MM_dd.parse(lFecAlta).getTime());
	...
	public static String obtieneValorPropiedad(
		...
		if (lstCatalogoTO != null) {
			ListIterator<CatalogoTO> listIterator = lstCatalogoTO
					.listIterator();
			while (listIterator.hasNext()) {
				catalogoTO = listIterator.next();
				if (catalogoTO.getIdVariable() == null
	...
	public static int comparaFechasDistribuidores(Vector<String> vFechas,
			int indice1, int indice2) throws CAException {
		String lFecAlta1 = "";
		String lFecAlta2 = "";
		try {
	...
	public static long diferenciaDiasCertificado(String _fecAlta) {
		...
		try {
			Locale currentLocale = new Locale("en", "US");
			SimpleDateFormat dateFormatAlta = new SimpleDateFormat("ddMMyyyy",
					currentLocale);
			java.util.Date dFecAlta = dateFormatAlta.parse(_fecAlta);
			java.util.Date dFecHoy = new java.util.Date();
			lDifDias = dFecHoy.getTime() - dFecAlta.getTime();
